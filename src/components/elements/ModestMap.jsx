...
const ModestMaps = () => {
   ...
   /*eslint-disable */
     ...
     *
     */
    MM.Template = function(template, subdomains = ['a', 'b', 'c']) {
    ...
    MM.MouseWheelHandler = function() {
        ...
        function mouseWheel(e) {
            ...
            // limit mousewheeling to once every 100ms
            var timeSince = new Date().getTime() - prevTime;
            var point = MM.getMousePoint(e, map);

            if (Math.abs(delta) > 0 && (timeSince > 100) && !precise) {
        ...
        handler.init = function(x) {
            map = x;
            document.querySelectorAll(".mapZoomDiv").forEach(el => el.remove());
            _zoomDiv = document.body.appendChild(document.createElement('div'));
            _zoomDiv.classList.add("mapZoomDiv");
    ...
    MM.DragHandler = function(onPan = undefined) {
        var handler = {},
            prevMouse,
            map;
    ...
    MM.RequestManager = function(name) {
        this.name = name;
    ...
    MM.RequestManager.prototype = {
        // Name of the layer which made the request
        name: '',
        ...
        requestTile: function(id, coord, url) {
            if (!(id in this.requestsById)) {
                var request = { id: id, coord: coord.copy(), url: url, name: this.name };
            ...
            while (this.openRequestCount < this.maxOpenRequests && this.requestQueue.length > 0) {
                ...
                if (request) {
                    ...
                    if(request.name !== undefined) {
                        img.classList.add(request.name);
                    }
    ...
    MM.Layer = function(provider, parent, name) {
        ...
        this.name = name;
        this.levels = {};
        this.requestManager = new MM.RequestManager(name);
        ...
        getTileComplete: function() {
            if (!this._tileComplete) {
                ...
                this._tileComplete = function(manager, tile) {
                    ...
                    tile.classList.add('map-tile-loaded');
                };
            }
    ...
    MM.MapCanvas = (parentMap) => {
        const canvas = document.createElement('canvas');
        canvas.style.position = 'absolute';
        canvas.style.left = '0';
        canvas.style.top = '0';
        canvas.width = parentMap.dimensions.x;
        canvas.height = parentMap.dimensions.y;
        parentMap.parent.appendChild(canvas);
    ...
    MM.Map.prototype = {
        ...
        swapLayersAt: function(i,j, index = '?') {
            /*eslint-disable */
