...
export class Zone {
  ...
  draw = (ctx, map) => {
    ...
    if (this.previewPoint) {
      const lastPoint = this.points[this.points.length - 1].ctxCoord(map);
      // ctx.setLineDash([5, 3]);
  ...
  linePreview(locPoint) {
    if (this.points && this.points.length) {
      this.previewPoint = locPoint;
    }
  }
   ...
   * @return {Object} An object which contains {stopIteration, stopPropagation} flags.
   */
  mouseMove = (ctxPoint, locPoint, isMouseDown, map, event, isDrawing) => {
